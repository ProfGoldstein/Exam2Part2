 ...
 * 
 * Added support for lollipop sticks
 * 
...
class DraggableShape {
	...
	public static final int STICK_WIDTH = 10;  //Added
	public static final int STICK_HEIGHT = 60;  //Added
	...
	private Point stickUpperLeft;  //Added
	private Color color;
	...
	public void paint(Graphics g) {
		stickUpperLeft = new Point(upperLeft);  //Added
		int delta = size / 2 - STICK_WIDTH / 2;  //Added
		stickUpperLeft.translate(delta, size);  //Added
		g.setColor(color);
		if (shape == CIRCLE) {
			if (isFilled) {
				g.fillOval(upperLeft.x, upperLeft.y, size, size);
				g.setColor(Color.white);  //Added
				g.fillRect(stickUpperLeft.x, stickUpperLeft.y, STICK_WIDTH, STICK_HEIGHT);	  //Added			
			} else {
	...
	public boolean contains(Point p) {
		...
		if (shape == CIRCLE) {
			...
			(p.x >= stickUpperLeft.x && p.x <= stickUpperLeft.x + STICK_WIDTH &&
					p.y >= stickUpperLeft.y && p.y <= stickUpperLeft.y + STICK_HEIGHT );  //Added
